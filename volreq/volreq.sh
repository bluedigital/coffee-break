...
function selectSrcDir() {
    ...
    if [ "$(cat /etc/fstab | grep $src_dir | awk '{ print $1 }' | grep -F $src_device)" = "" ]; then
        echo -ne "$color_red"
    ...
    case "$cloud_provider" in
        aws)
            declare -a srv_instance_identity=($( \
                ...
                egrep -e '"(availabilityZone|instanceId|region)"' | \
                tr -d '":,\n'))
...
function tarCloneFs() {
    ...
    local _src_dir=$(realpath -q $1)
    local _dst_dir=$(realpath -q $2)
