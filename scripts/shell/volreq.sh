...
function createDstDir() {
  ...
  *)
  ...
  if ! parted "$dst_block_device" -s -a optimal mkpart primary "$dst_volume_fs" 3 100% >/dev/null 2>>/tmp/parted.$$; then
    echo -ne "$color_bold"
  ...
  if ! mkfs -t "$dst_volume_fs" "$dst_volume_partition" >>/tmp/mkfs.$$ 2>&1; then
    echo -ne "$color_bold"
...
function tarCloneFs() {
  ...
  if ! (cd "$_src_dir" && tar cf - . 2>>/tmp/clone_fs_in.$$ | tar xvf - -C "$_dst_dir" 2>>/tmp/clone_fs_out.$$); then
    echo -ne "$color_bold"
